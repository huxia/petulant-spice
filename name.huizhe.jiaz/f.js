...
appClient.async('read', name, callback);
},
...
(function(){
var userAgent = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_8_2)AppleWebKit/537.36 (KHTML, like Gecko)Chrome/28.0.1500.71 Safari/537.36';
...
text = text.replace(/&#\d+;/g, function(w){
return String.fromCharCode(parseInt(w.replace(/\D/g, '')));
});
return text;
}
...
function _getStartPageIndex(url, cb){
var key = 'db_pa_start-' + url;
appClient.read('db_pa_start-' + url, function(value){
if(value){
cb(parseInt(value), 4);
return;
}
...
_setStartPageIndex(url, '0', function(){
cb(0,60);
});
return;
}
var rePageCount = />(\d+)<\/a>[\n\s]*<span class="?next"?/i;
var match = response.string.match(rePageCount);
if(!match){
_setStartPageIndex(url, '0', function(){
cb(0,60);
});
...
var totalPageCount = parseInt(match[1]);
var startPage = Math.max(0, totalPageCount - 200);
_setStartPageIndex(url, startPage + '', function(){
cb(startPage,60);
});
});
});
}
function _setStartPageIndex(url, value, cb){
var key = 'db_pa_start-' + url;
appClient.write('db_pa_start-' + url, value + '', cb);
}
...
var funcs = [];
var pictures = [];
for(var i=start_page*page_size;i<(start_page+fetch_page_count)*page_size;i+=page_size){
(function(){
var start = i;
var fetch_url = url + '?start=' + start;
var fetch_refer = start < page_size ? 'http://www.douban.com/' : (url + '?start=' + (start - page_size));
...
if(response.statusCode / 100 != 2){
cb('error: code ' + response.statusCode);
return;
}
var valid = false;
var reImg = / class="photolst_photo" title="([^"]*)">[\s\n]*<img src="([^"]*)"/ig;
var match;
while(match = reImg.exec(response.string)){
var imgSrc = decodeHTMLEntities(match[2]);
var reImgSrc = /thumb/;
var reImgId = /(\d+)\.\w+$/;
...
headers: {
'User-Agent': userAgent,
'Refer': imgSrc.match(reImgId)?
('http://www.douban.com/photos/photo/' + imgSrc.match(reImgId)[1] + '/large'):
fetch_url
}
},
backupRequest: {
url: imgSrc.replace(reImgSrc, 'photo'),
headers: {
'User-Agent': userAgent,
'Refer': imgSrc.match(reImgId)?
('http://www.douban.com/photos/photo/' + imgSrc.match(reImgId)[1] + '/'):
fetch_url
}
},
...
if (valid){
_setStartPageIndex(url, start + '', function(){
cb(null);
});
}else{
cb('empty');
...
if(err == 'empty')err = null;
callback(pictures, err);
});
});
};
...
appClient.fetchDone(totalFetchedNew, firstErr);
	...
	appClient.beginFetch('name.huizhe.jiaz', {
		urls: [
			'http://www.douban.com/photos/album/38989541/#nolarge',
			'http://www.douban.com/photos/album/38870647/',
			'http://www.douban.com/photos/album/46541475/'
		]
	});
